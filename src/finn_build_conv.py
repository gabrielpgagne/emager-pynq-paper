...
def get_and_save_best_model(subject: int, quant: int, transform_name: str):
    """
...
def launch_finn_build():
    ...
    cmd = [
        ...
        f"./run-docker.sh build_custom {proj_dir} src/_build_dataflow_conv",
    ]
    try:
...
if __name__ == "__main__":
    ...
    for sub in subjects:
        procs = []
        for quant in quants:
            get_and_save_best_model(sub, quant, globals.TRANSFORM)
            utils.lock_finn(subject=sub, quant=quant, shots=-1)
