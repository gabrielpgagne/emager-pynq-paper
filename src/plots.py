...
def raw_acc_vs_maj_acc(base_dir, shots, quants):
    ...
    for q in quants:
        ...
        print(
            ("\\multirow{2}{*}{%s} & Intra & " % q) + f"$ {100*np.mean(acc_pairs_intra[:,0:1], axis=0).item():.1f} \\pm {100*np.std(acc_pairs_intra[:,0:1], axis=0).item():.1f} $ & $ {100*np.mean(acc_pairs_intra[:,1:], axis=0).item():.1f} \\pm {100*np.std(acc_pairs_intra[:,1:], axis=0).item():.1f} $" + r" \\ \cline{2-2}"
        )
        print(
            "                   & Inter & " + f"$ {100*np.mean(acc_pairs_inter[:,0:1], axis=0).item():.1f} \\pm {100*np.std(acc_pairs_inter[:,0:1], axis=0).item():.1f} $ & $ {100*np.mean(acc_pairs_inter[:,1:], axis=0).item():.1f} \\pm {100*np.std(acc_pairs_inter[:,1:], axis=0).item():.1f} $" + r" \\ \cline{1-4}"
        )
...
def plot_processed(dataset_root, subject, session, rep, channel):
    ...
    datap = eq.nroot_c(datap, 3.0, 10000)
    timep = np.linspace(
...
def plot_quantization():
    ...
    qdata = eq.nroot_c(data, 3.0, 10000)
